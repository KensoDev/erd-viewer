...
func (e *PlantUMLExporter) Export(schema *db.SchemaData, selectedTables []string) (string, error) {
...
func (e *PlantUMLExporter) buildEntity(table db.Table) string {
	var sb strings.Builder

	sb.WriteString(fmt.Sprintf("entity \"%s\" as %s {\n", table.Name, sanitizeIdentifier(table.Name)))

	// Group primary keys first
	var primaryKeys []db.Column
	var regularColumns []db.Column
...
func (e *PlantUMLExporter) buildColumn(col db.Column, isPK bool) string {
	var parts []string
...
func (e *PlantUMLExporter) buildRelationship(fk db.ForeignKey) string {
